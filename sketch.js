...
var COLORS = {
  ...
  TREE: '#005108',
  FOG:  '#005108',
  LIGHT:  { road: '#434343', grass: '#EDD094', rumble: '#555555', lane: '#FFEF1C' },
...
function updateGame() {
  ...
  if (keyFaster)
    spd = accelerate(spd, accel, dt);
  else if (keySlower)
    spd = accelerate(spd, braking, dt);
  else
...
function render() {
  ...
  for (var i = 0; i < drawDistance; i++) {
    ...
    if (segment.p1.z <= cameraDepth || segment.p2.screen.y >= segment.p1.screen.y || segment.p2.screen.y >= maxY) {
      continue;
    }
    
    renderSegment(gameWidth, lanes,
                  segment.p1.screen.x,
                  segment.p1.screen.y,
                  segment.p1.screen.w,
                  segment.p2.screen.x,
